...
func ConDb() (*xorm.Engine, error) {
	...
	case dbtype == "pgsql":
		...
		//return xorm.NewEngine("postgres", "host=110.76.39.205 user=postgres password=jn!@#9^&* dbname=pgsql sslmode=disable")
		return xorm.NewEngine("postgres", "user=postgres password=jn!@#9^&* dbname=flower sslmode=disable")
		//return xorm.NewEngine("postgres", "host=127.0.0.1 port=6432 user=postgres password=jn!@#9^&* dbname=flower sslmode=disable")
	}
...
func initData() {
	...
	if usr, err := GetUserByRole(-1000); usr == nil && err == nil {
		if id, err := AddUser("insion@live.com", "root", "root", "root", helper.Encrypt_hash("rootpass", nil), -1000); err == nil {
			fmt.Println("Default Email:insion@live.com ,Username:root ,Password:rootpass Userid:", id)
		} else {
			fmt.Print("create root got errors:", err)
		}

	}
	fmt.Println("The flower system has started!\nCopyright © 2014 Lihuaer.com, Proudly powered by martini.")
}
